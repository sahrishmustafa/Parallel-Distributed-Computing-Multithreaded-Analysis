...
int isValidWord(const char *word) {
    ...
    for (int i = 0; word[i]; i++) {
        if (isalpha(word[i])) {
            ...
            return 0; // number detected - rejected
        }
    }
    return hasAlpha; 
}
...
void distinguishWord(char *word) {
    ...
    for (int i = 0; word[i]; i++) {
        if (isalpha(word[i])) {
            ...
            if (index > 0) {
                cleanWord[index] = '\0'; // split the word at special character
...
// Output Verification
void storeOutput() {
    FILE *outputFile = fopen("freqs_word.txt", "w");
    if (!outputFile) {
...
int main() {
    ...
    char combinedDir[] = "text/en/combined_folder";  // Hardcoded path ~ 100,000 files .snt
